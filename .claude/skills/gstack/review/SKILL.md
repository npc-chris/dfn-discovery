...
- Repository context: DFN uses Next.js frontend (frontend/) and Express + Drizzle backend (backend/).
- Keep review scope tight to the user request and changed files.
- Prioritize production-risk findings first: auth, payments, data integrity, security, and regressions.
- Do not broaden into unrelated refactors unless explicitly requested.
- If no high-severity findings exist, call out residual test or coverage gaps briefly.

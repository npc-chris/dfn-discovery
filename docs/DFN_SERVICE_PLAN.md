...
- human review only where the risk is real
...
1. AI as an offline analyst. It processes job requests, survey data, and research in the background, then writes structured outputs for the rest of the product. Completeness: 9/10.
2. AI as a decision explainer. It takes the scores from the core engine and turns them into readable briefs, tradeoffs, and next steps. Completeness: 9/10.
3. AI as an isolated planner. It receives a sanitized job spec and generates a routing, sourcing, or validation plan. Completeness: 8/10.
4. AI as a chatbot. Users ask anything and the model tries to answer. Completeness: 3/10.

RECOMMENDATION: Choose options 1 and 2 first, then add 3 if the core workflow is stable. Skip 4. The chatbot path looks flexible, but it leaks trust fast and makes the product feel fuzzy.
...
These are real features, but they are not the first wedge. They add surface area fast and do not prove the core value faster.
...
If a feature does not help a user decide, route, source, price, or de-risk a manufacturing job, it is noise right now.
...
If the current plan includes any of these, cut them:

...
export async function extractJobFields(_job: Job): Promise<ExtractionResult> {
  // TODO: call AI model with sanitized job payload
  // Extract: process_type, material_type, volume_band, etc.
  throw new Error('Not implemented');
}

export async function summarizeFactory(_factoryId: string): Promise<string> {
  // TODO: summarize a factory profile into a decision brief
  throw new Error('Not implemented');
}

export async function explainRanking(_jobId: string, _factoryId: string): Promise<string> {
  // TODO: explain why this factory ranks higher than others
  throw new Error('Not implemented');
}

export async function flagAnomalies(_jobId: string): Promise<string[]> {
